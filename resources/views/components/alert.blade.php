...
<div class="alert-{{ $type }} {{ $color['bg'] }} {{ $color['border'] }} {{ $color['text'] }} rounded-lg border px-4 py-3 shadow-sm {{ $class }}" role="alert">
    <div class="flex">
        ...
        <div class="ml-3 flex-1">
            @if ($title)
                ...
                <p class="mt-1 text-sm">{{ $message ?: $slot }}</p>
            @else
                <p class="text-sm font-medium">{{ $message ?: $slot }}</p>
            @endif
        </div>
        @if ($dismissible)
            <button type="button" class="ml-3 inline-flex text-opacity-70 hover:text-opacity-100" onclick="this.parentElement.parentElement.remove()">
