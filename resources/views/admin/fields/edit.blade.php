...
@section('content')
    <div class="max-w-2xl">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Edit Lapangan</h1>
            <p class="text-gray-600 mt-2">Ubah informasi lapangan futsal</p>
        </div>

        <!-- Form Card -->
        <x-card>
            <form method="POST" action="{{ route('admin.fields.update', $field) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <x-form.textarea 
                    ...
                    :value="old('description', $field->description ?? '')"
                    rows="4"
                />
                ...
                <x-form.checkbox 
                    ...
                    :checked="old('is_active', $field->is_active) ? true : false"
                    value="1"
                />
